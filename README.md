# Hackathon Soberania Digital 🚀

**Um evento colaborativo para construir alternativas tecnológicas livres e populares**

> *"Se quem cria conhecimento não decide seu uso, a tecnologia vira arma das grandes empresas, não da maioria."*  
> — Manifesto do CPDD

## 📋 Sobre o Hackathon

O CPDD (Coletivo Popular de Design e Desenvolvimento) e a Soberana convidam para o primeiro hackathon popular focado em **soberania digital, segurança de dados e autonomia tecnológica**. 

Em resposta à dependência das Big Techs e ao colonialismo digital, este evento busca construir coletivamente ferramentas que fortaleçam a organização popular e a resistência contra a vigilância corporativa e estatal.

## 🔗 Links Úteis

| Recurso | Descrição | Link |
|---------|-----------|------|
| 📝 Formulário de Inscrição | Inscreva-se individualmente ou em equipe | [Acessar](https://forms.gle/n1rCNBMuSQD28yUC8) |
| 💬 Discord da Soberana | Encontre equipe e tire dúvidas | [Participar](https://discord.gg/vDrDtV4KbS) |
| 🐙 Repositório Oficial | Envie seu projeto e documentação | [Acessar](https://github.com/Coletivo-Popular-Design-Desenvolvimento/hackathon-cppd-soberana) |
| 🎤 Palco de Abertura | Cerimônia inicial do evento | [Participar]() |
| 📚 Projetos de Exemplo | Ideias e inspirações | [Ver projetos](https://github.com/Coletivo-Popular-Design-Desenvolvimento/hackathon-cppd-soberana/blob/main/Exemplos%20de%20projetos.md) |
| 📊 Critérios de Revisão | Linhas base para os projetos | [Ver critérios](https://github.com/Coletivo-Popular-Design-Desenvolvimento/hackathon-cppd-soberana/blob/main/Crit%C3%A9rios%20de%20Revis%C3%A3o.md) |

## 🎯 Tema Central

**Soberania digital, segurança de dados e autonomia tecnológica**

Buscamos alternativas para ferramentas como:
- Google Drive → Armazenamento descentralizado
- Discord/WhatsApp → Comunicação segura
- Notion → Gestão de conhecimento
- Figma → Design colaborativo

## 🛡️ Requisitos dos Projetos

Os projetos devem atender a pelo menos um dos seguintes pilares:

- **🤝 Soberania**: Controle sobre infraestrutura e dados
- **🔒 Segurança**: Criptografia e proteção contra vigilância  
- **🎯 Controle de Acesso**: Gestão de permissões por contexto
- **📊 Auditoria**: Rastreabilidade e transparência

## 👥 Como Participar

### 1. **Forme sua equipe**
- Individual ou em grupos de até 4 pessoas
- Não é necessário saber programar
- [Encontre equipe no Discord]()

### 2. **Inscreva-se**
- Preencha o [formulário de inscrição](https://forms.gle/n1rCNBMuSQD28yUC8)
- Participe do palco de abertura (obrigatório para pelo menos um membro da equipe)

### 3. **Desenvolva seu projeto**
- Podendo ser, mas não exclusivo: Estudo técnico, Prova de Conceito, MVP
- Use tecnologias livres e éticas
- Documente no arquivo `conceito.md`

### 4. **Submeta até o prazo**
- Siga o [tutorial de submissão](https://github.com/Coletivo-Popular-Design-Desenvolvimento/hackathon-cppd-soberana/blob/main/Tutorial%20Github.md)
- Adicione sua branch ao repositório
- Suba ou crie seus arquivos
- Crie sua pull request

_Obs: Alternativamente, você também pode enviar seu projeto através do [formulário](https://docs.google.com/forms/d/e/1FAIpQLSdhYAAAu39WLI7FB1DzoDX2AgKmBogvqS-2zbTmWAloZF9UXg/viewform)._

## 📅 Cronograma

| Data | Evento |
|------|--------|
| 21/11/2025 | Divulgação do material e abertura das inscrições |
| 28/11/2025 20:00 | Palco de abertura |
| 28/11/2025 22:00 | Encerramento das inscrições |
| 30/11/2025 12:00 | Anuncio de encerramento |
| 14/12/2025 | Apresentação dos projetos destacados |

## ⚖️ Critérios de Revisão

- 🎯 Pertinência ao tema
- 🔥 Alinhamento ideológico 
- 🆓 Tecnologias livres
- ⚙️ Viabilidade técnica
- 📢 Capacidade de mobilização
- 💡 Criatividade e inovação
- 📚 Documentação

[Ver critérios completos](https://github.com/Coletivo-Popular-Design-Desenvolvimento/hackathon-cppd-soberana/blob/main/Crit%C3%A9rios%20de%20Revis%C3%A3o.md)

## 💡 Ideias de Projetos

Explore nossas sugestões para inspirar seu projeto:

### Para Agitação e Propaganda
- Redes IPFS comunitárias
- Plataformas de comunicação massiva
- Ferramentas de distribuição de conteúdo

### Para Organização Interna  
- Sistemas de mensagens criptografadas
- Ferramentas de colaboração seguras
- Gestão de documentos sensíveis

[Ver lista completa de projetos exemplo](https://github.com/Coletivo-Popular-Design-Desenvolvimento/hackathon-cppd-soberana/blob/main/Exemplos%20de%20projetos.md)

## 🛠️ Tecnologias de Exemplo

### Comunicação
- Matrix/Element, XMPP, SimpleX, Session, Revolt

### Armazenamento
- Nextcloud, Seafile, IPFS, CryptPad

### Colaboração
- Jitsi, Gitea, AppFlowy, CryptPad

## ❓ Precisa de Ajuda?

- **Dúvidas técnicas**: Canal #dúvidas no Discord
- **Formação de equipes**: Fórum dedicado no Discord  
- **Suporte CPDD/Soberana**: Equipes disponíveis durante todo evento

## 🏆 Reconhecimento

Todos os participantes que submeterem projetos receberão:
- 📜 Certificado digital de participação
- 🔍 Visibilidade para projetos destacados
- 🚀 Acompanhamento para projetos promissores

## 📝 Documentação do Projeto

Cada projeto deve conter na raiz:
/conceito.md
/README.md (opcional para documentação técnica)
/código/ (se aplicável)
/documentação/ (se aplicável)

**Arquivo `conceito.md` obrigatório** com:
- Descrição do projeto
- Tecnologias utilizadas
- Como atende aos requisitos
- Descobertas e aprendizados

---

**Vamos juntos construir tecnologias que servem ao povo, não ao capital!**

*CPDD & Soberana - Pela soberania tecnológica da classe trabalhadora*
